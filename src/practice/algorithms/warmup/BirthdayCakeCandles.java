...
public class BirthdayCakeCandles {
    ...
    static int birthdayCakeCandles(int[] ar) {
        int maxCandle = Arrays.stream(ar).max().getAsInt();
        int blowingCandles = ((int) Arrays.stream(ar).filter(i -> i == maxCandle).count());
        System.out.println(blowingCandles);
        return blowingCandles;
    }

    public static void main(String[] args) {
        birthdayCakeCandles(new int[]{3 ,2 ,5,2,2,2,24,4,4,44,44,1 ,44,44,3});
    }
}
